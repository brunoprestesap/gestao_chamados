...
import type { BreachByUnit } from '@/lib/sla-breach-report';
...
export function BreachByUnitTable({ data }: { data: BreachByUnit[] }) {
  if (data.length === 0) {
    return (
      <p className="py-8 text-center text-sm text-muted-foreground">
        Nenhum breach associado a unidades no período.
      </p>
    );
  }
          ...
          {data.map((u) => (
            <tr
              ...
              className={cn(
                ...
                u.breachRate > 30 && 'bg-red-50/50 dark:bg-red-900/10',
              )}
            >
              <td className="px-4 py-3 font-medium">{u.unitName}</td>
              <td className="px-3 py-3 text-right tabular-nums">{u.totalChamados}</td>
              <td className="px-3 py-3 text-right tabular-nums">{u.responseBreaches}</td>
              <td className="px-3 py-3 text-right tabular-nums">{u.resolutionBreaches}</td>
              ...
              <td className="hidden px-3 py-3 md:table-cell">
                <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted/60">
                  <div
                    className={cn('h-full rounded-full transition-all', barColor(u.breachRate))}
                    style={{ width: `${Math.min(100, u.breachRate)}%` }}
                  />
                </div>
              </td>
            </tr>
