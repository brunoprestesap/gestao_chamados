...
- [ ] Registrar uma cotação para terceiros no mesmo chamado (dispara o mesmo evento de pausa, com outro motivo) → a conversa do solicitante não muda por causa desse evento → AC-3 — **não exercido em 2026-09-23**: não achei, no tempo do `/check verify`, um caminho de UI que dispare esse fluxo (o motivo de pausa que abre a cotação não apareceu como opção para o técnico, e o Preposto não acessa o painel de ações de `/chamados-atribuidos/[id]`). Confirmado só por leitura de código (os textos dos dois motivos de pausa são diferentes, e o manipulador compara texto exato). Engenheiro decidiu adiar; retomar quando achar o gatilho real da cotação.
