...
- **AC-14**: Toda requisição ao vLLM leva a amostragem inteira no corpo: `temperature`, `top_p`, `top_k`, `min_p`, `presence_penalty`, `frequency_penalty`, `repetition_penalty` e `max_tokens`. Sem `sampling`, os valores são os de `LLM_SAMPLING_DEFAULTS`: 0.7, 0.8, 20, 0, 0, 0, 1 e 512 (o `presencePenalty` e o `maxOutputTokens` só mudam pelas regras de ajuste das tarefas 20 e 21 do build plan). O parâmetro `sampling` sobrescreve só os campos passados entre `temperature`, `topP`, `topK`, `minP`, `presencePenalty` e `maxOutputTokens`; `frequency_penalty` e `repetition_penalty` são fixos. Um valor fora das faixas falha como `bad_request` sem chegar ao vLLM.
...
**Amostragem** (revisão de 2026-09-17): segue o card do Qwen3 para o modo sem thinking (`temperature 0.7`, `top_p 0.8`, `top_k 20`, `min_p 0`), com `presence_penalty 0` e `maxOutputTokens 512`. Tudo vai explícito em todo pedido, sem depender dos padrões do servidor. O teste de fumaça mede a velocidade e compara três configurações antes de fixar os valores.
...
| idem                                   | amostragem                                            | `LLM_SAMPLING_DEFAULTS` em `lib/llm/config.ts`: `temperature 0.7`, `topP 0.8`, `topK 20`, `minP 0`, `presencePenalty 0`, `maxOutputTokens 512`, mais os fixos `frequencyPenalty 0` e `repetitionPenalty 1`; o parâmetro `sampling` sobrescreve campo a campo. `presencePenalty` e `maxOutputTokens` só mudam pelas regras da tarefa 20 e da tarefa 21 |
...
- Amostragem no corpo: sem `sampling`, o servidor falso recebe `temperature: 0.7`, `top_p: 0.8`, `top_k: 20`, `min_p: 0`, `presence_penalty: 0`, `frequency_penalty: 0`, `repetition_penalty: 1` e `max_tokens: 512`, nas duas funções; com `sampling: { topK: 40, maxOutputTokens: 200 }`, só esses dois mudam (tarefa 19). O `LlmCall` guarda os valores efetivos (asserção acrescentada na tarefa 22). Verifica **AC-14**, **AC-15**.
...
- Um laço de repetição termina em 512 tokens como `invalid_output`, dentro do prazo, e não abre o disjuntor para todos; o `finishReason` mostra no registro e no painel da funcionalidade 18 quantos cortes houve.
...
- 512 tokens podem ser pouco para uma resposta de conversa com pergunta e motivo; a funcionalidade que precisar de mais passa o próprio `maxOutputTokens` e confere que cabe no prazo da raia.
...
- [ ] Nas specs das funcionalidades 11 e 12: definir o `maxOutputTokens` da tarefa se a resposta passar de 512 tokens, conferindo com a velocidade medida na tarefa 20.
