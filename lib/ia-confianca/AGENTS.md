...
- **`limiteConfianca: null` significa autonomia impossível para aquele campo.** Nesta fatia nada além da configuração lê `autonomiaAtiva` nem os limites; fatias futuras que forem ler devem tratar `null` como limite inatingível.
